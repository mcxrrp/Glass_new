...
def onResize():
    """Resize dock."""
    mdi = mw.findChild(QtGui.QMdiArea)
    active_workbench = str(Gui.activeWorkbench().__class__.__name__)
    active_view = str(Gui.activeView())
    if active_view == "<View3DInventor>":
        active_view = "View3DInventor"

    if mode == 1:
        x = 0
        y = 0
        w = mdi.geometry().width() / 100 * 20
        h = mdi.geometry().height() - mdi.findChild(QtGui.QTabBar).geometry().height()
    ...
    if active_view == "View3DInventor" and active_workbench == "StartWorkbench":
        ...
        #print('a_w v3dI stwb')
        return
    if active_view == "MDI view page":
        dock.setVisible(True)
        #print ('a_v MDIvP')
        return     

    #if active_workbench != "TechDrawWorkbench":
    #    dock.setVisible(True)
    #else:
    #    dock.setVisible(True)
...
timer = QtCore.QTimer()
timer.timeout.connect(onStart)
timer.start(9000)  # Adjust the timeout interval as needed
